...
import openwakeword
from data import generate_adversarial_texts, augment_clips, mmap_batch_generator, embedding_clips, only_augment_clips
...
class Model(nn.Module):
    def __init__(self, n_classes=1, input_shape=(16, 96), model_type="dnn",
                 layer_dim=128, n_blocks=1, seconds_per_example=None, checkpoint_path="/kaggle/working/checkpoints"):
        super().__init__()
        ...
        if model_type == "dnn":
            ...
            class CNNBlock(nn.Module):
                def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, dropout_p=0.1):
                    super().__init__()
                    self.conv = nn.Conv1d(in_channels, out_channels, kernel_size=kernel_size, stride=stride, padding='same')
            ...
            class Net(nn.Module):
                def __init__(self, input_shape, layer_dim=128, n_classes=1):
                    super().__init__()
                    
                    num_features = input_shape[1]
                    self.cnn_blocks = nn.Sequential(
                        CNNBlock(in_channels=num_features, out_channels=64, kernel_size=3),
                        CNNBlock(in_channels=64, out_channels=128, kernel_size=3),
                        CNNBlock(in_channels=128, out_channels=128, kernel_size=3)
                    )
                    
                    final_cnn_output_size = 128 * (input_shape[0] // (2**3))
                    
                    self.flatten = nn.Flatten()
                    self.dense1 = nn.Linear(final_cnn_output_size, layer_dim)
                    self.relu = nn.ReLU()
                    self.dropout = nn.Dropout(p=0.2)
                    self.dense2 = nn.Linear(layer_dim, n_classes)
                ...
                def forward(self, x):
                    ...
                    return x
            self.model = Net(input_shape, layer_dim, n_classes=n_classes)
    ...
    def auto_train(self, X_train, X_val, false_positive_val_data, steps=50000, max_negative_weight=1000,
        ...
        with torch.no_grad():
            # Collect tất cả predictions và labels từ X_val
            all_val_predictions = []
            all_val_labels = []
            ...
            for batch in X_val:
                ...
                all_val_predictions.append(val_ps)
                all_val_labels.append(y)
            
            # Concat tất cả batches lại
            all_val_predictions = torch.cat(all_val_predictions, dim=0)
            all_val_labels = torch.cat(all_val_labels, dim=0)
            
            # Tính metrics trên TOÀN BỘ validation set
            combined_model_recall = self.recall(all_val_predictions, all_val_labels[..., None]).detach().cpu().numpy()
            combined_model_accuracy = self.accuracy(all_val_predictions, all_val_labels[..., None].to(torch.int64)).detach().cpu().numpy()
    ...
    def re_auto_train(self, X_train, X_val, false_positive_val_data, steps=50000, start_phase=1, start_step=0, max_negative_weight=1000,
                   target_fp_per_hour=0.2):
        """A sequence of training steps that produce relatively strong models
        automatically, based on validation data and performance targets provided.
        After training merges the best checkpoints and returns a single model.
    ...
    def train_model(self, X, max_steps, warmup_steps, hold_steps, phase, start_step=0, X_val=None,
        ...
        for step_ndx, data in tqdm(enumerate(X, start=start_step), total=max_steps, desc="Training"):
            logging.info(str(step_ndx))
            print(str(step_ndx))
            ...
            if step_ndx in val_steps and step_ndx > 1 and X_val is not None:
                ...
                for val_step_ndx, data in enumerate(X_val):
                    with torch.no_grad():
                        ...
                        # val_recalls.append(self.recall(val_predictions, y_val[..., None]).detach().cpu().numpy())
                        # val_accs.append(self.accuracy(val_predictions, y_val[..., None].to(torch.int64)).detach().cpu().numpy())
                        # val_fps.append(self.fp(val_predictions, y_val[..., None]).detach().cpu().numpy())
                ...
                # Tính metrics trên TOÀN BỘ validation set
                self.history["val_accuracy"].append(self.recall(all_val_predictions, all_val_labels[..., None]).detach().cpu().numpy())
                self.history["val_recall"].append(self.accuracy(all_val_predictions, all_val_labels[..., None].to(torch.int64)).detach().cpu().numpy())
                self.history["val_n_fp"].append(self.fp(all_val_predictions, all_val_labels[..., None]).detach().cpu().numpy())

                # Lưu TRUNG BÌNH của tất cả batch
                # self.history["val_accuracy"].append(np.mean(val_accs))
                # self.history["val_recall"].append(np.mean(val_recalls))
                # self.history["val_n_fp"].append(np.sum(val_fps))  # hoặc np.mean tùy mục đích
            ...
            if step_ndx in val_steps and step_ndx > 1:
                if self.history["val_n_fp"][-1] <= np.percentile(self.history["val_n_fp"], 50) and \
                   self.history["val_recall"][-1] >= np.percentile(self.history["val_recall"], 5):
                    ...
                    checkpoint_path = self.checkpoint_dir / f"phase_{phase:02d}_step_{step_ndx:05d}_recall_{self.best_val_recall:.4f}.pt"
                    torch.save({
                        "phase": phase,
                        "step": step_ndx,
                        "model_state_dict": self.model.state_dict(),
                        "optimizer_state_dict": self.optimizer.state_dict(),
                        "val_recall": self.best_val_recall,
                        "val_accuracy": self.best_val_accuracy,
                        "val_fp_per_hr": self.history.get("val_fp_per_hr", [0])[-1]
                    }, checkpoint_path)
                    logging.info(f"✅ Saved checkpoint: {checkpoint_path}")

                    # Ghi log metrics vào file CSV (tùy chọn)
                    with open(self.checkpoint_dir / "metrics_log.csv", "a") as f:
                        if f.tell() == 0:
                            f.write("phase,step,val_recall,val_accuracy,val_fp_per_hr\n")
                        current_val_fp_per_hr = self.history.get("val_fp_per_hr", [0])[-1]
                        f.write(f"{phase},{step_ndx},{self.best_val_recall},{self.best_val_accuracy},{current_val_fp_per_hr}\n")

            if step_ndx == max_steps-1:
                break

    def get_false_positive(self,false_positive_val_data):
        combined_model = self.model
        val_set_hrs = 11.3
        with torch.no_grad():
            combined_model_fp = 0
            ...
            combined_model_fp_per_hr = (combined_model_fp/val_set_hrs).detach().cpu().numpy()
        print(combined_model_fp_per_hr)

    def export_from_checkpoint(self, checkpoint_path, output_path, class_mapping="output"):
        """
        ✅ Load checkpoint đã lưu rồi export sang ONNX
        Args:
            checkpoint_path (str | Path): đường dẫn tới file .pt chứa model_state_dict
            output_path (str | Path): đường dẫn lưu file .onnx
            class_mapping (str): tên output node trong onnx
        """
        checkpoint_path = Path(checkpoint_path)
        output_path = Path(output_path)
        assert checkpoint_path.exists(), f"Checkpoint {checkpoint_path} không tồn tại"

        # Load checkpoint
        checkpoint = torch.load(checkpoint_path, map_location="cpu", weights_only=False)

        # Khôi phục lại trọng số
        if "model_state_dict" in checkpoint:
            self.model.load_state_dict(checkpoint["model_state_dict"])
        else:
            self.model = checkpoint  # Trường hợp checkpoint là model trực tiếp (torch.save(self.model))

        self.model.eval()

        # Xuất ONNX
        dummy_input = torch.rand(self.input_shape)[None, ]
        if self.n_classes == 1:
            torch.onnx.export(
                self.model.to("cpu"),
                dummy_input,
            ...
            export_model = ExportModel(self.model.to("cpu"))
            torch.onnx.export(
                export_model,
                dummy_input,
                output_path,
        ...
        logging.info(f"✅ Exported ONNX model: {output_path}")
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--export_checkpoint",
        help="Export a trained checkpoint (.pt) to ONNX format",
        action="store_true",
        default=False,
        required=False
    )
    parser.add_argument(
        "--checkpoint_path",
        help="Path to the checkpoint file (.pt) to export",
        type=str,
        required=False
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default="narrow",
        help="Chọn cấu hình SNR: base | narrow"
    )
    ...
    positive_train_output_dir = config["positive_train_output_dir"]
    positive_test_output_dir = config["positive_test_output_dir"]
    negative_train_output_dir = config["negative_train_output_dir"]
    negative_test_output_dir = config["negative_test_output_dir"]

    box_positive_train_output_dir = config["box_positive_train_output_dir"]
    box_positive_test_output_dir = config["box_positive_test_output_dir"]
    box_negative_train_output_dir = config["box_negative_train_output_dir"]
    box_negative_test_output_dir = config["box_negative_test_output_dir"]
    ...
    # background_paths = []
    # if len(config["noise_paths_duplicate_rate"]) != len(config["noise_paths"]):
    #     config["noise_paths_duplicate_rate"] = [1]*len(config["noise_paths"])
    # for background_path, duplication_rate in zip(config["noise_paths"], config["noise_paths_duplicate_rate"]):
    #     print(config["noise_paths"][background_path][0])
    #     print(config["noise_paths_duplicate_rate"][duplication_rate])
    #     background_paths.extend([i.path for i in os.scandir(config["noise_paths"][background_path][0])]*config["noise_paths_duplicate_rate"][duplication_rate])
    ...
    if args.augment_clips is True:
        snr_ranges = snr_configs[args.snr_mode]
        # if config['save_raw_positive_test']:
            # shutil.copytree(positive_test_output_dir, config['save_raw_positive_test'], dirs_exist_ok=True)
            # shutil.copytree(negative_test_output_dir, config['save_raw_negative_test'], dirs_exist_ok=True)

        # if not os.path.exists(os.path.join(feature_save_dir, "positive_features_trai2.npy")) or args.overwrite is True:
        #     positive_clips_train = [str(i) for i in Path(positive_train_output_dir).rglob("*.wav")]*config["augmentation_rounds"]
        #     random.seed(42)  # (tùy chọn) để tái lập kết quả, nếu không cần thì bỏ dòng này
        #     random.shuffle(positive_clips_train)
        #     # positive_clips_train
        #     positive_clips_train_data_generator = augment_clips(positive_clips_train, total_length=config["total_length"],
        #                                                    batch_size=config["augmentation_batch_size"],
        #                                                 #    background_clip_paths=background_paths,
        #                                                    RIR_paths=rir_paths,
        #                                                    noise_paths=config["noise_paths_train"],
        #                                                    augmentation_probabilities=config["augmentation_probabilities"],
        #                                                    noise_type_probabilities=config["noise_type_probabilities"],
        #                                                    save_debug_path=config['save_debug_path_positive_train'],
        ...
        #     else:
        #         n_cpus = n_cpus//2
        #     compute_features_from_generator(positive_clips_train_data_generator, n_total=len([str(i) for i in Path(positive_train_output_dir).rglob("*.wav")])*config["augmentation_rounds"],
        ...
        # if not os.path.exists(os.path.join(feature_save_dir, "positive_features_test2.npy")) or args.overwrite is True:
        #     positive_clips_test = [str(i) for i in Path(positive_test_output_dir).rglob("*.wav")]*config["test_augmentation_rounds"]
        #     random.seed(42)  # (tùy chọn) để tái lập kết quả, nếu không cần thì bỏ dòng này
        #     random.shuffle(positive_clips_test)
        #     # positive_clips_test_generator
        #     positive_clips_test_data_generator = augment_clips(positive_clips_test, total_length=config["total_length"],
        #                                                   batch_size=config["augmentation_batch_size"],
        #                                                 #   background_clip_paths=background_paths,
        #                                                   RIR_paths=rir_paths,
        #                                                   noise_paths=config["noise_paths_test"],
        #                                                   augmentation_probabilities=config["augmentation_probabilities"],
        #                                                   noise_type_probabilities=config["noise_type_probabilities"],
        #                                                   save_debug_path=config['save_debug_path_positive_test'],
        #                                                    snr_ranges=snr_ranges)
        #     logging.info("#"*50 + "\nComputing openwakeword features for generated samples\n" + "#"*50)
        #     n_cpus = os.cpu_count()
        #     if n_cpus is None:
        ...
        #     else:
        #         n_cpus = n_cpus//2
        #     compute_features_from_generator(positive_clips_test_data_generator, n_total=len([str(i) for i in Path(positive_test_output_dir).rglob("*.wav")])*config["test_augmentation_rounds"],
        #                                     clip_duration=config["total_length"],
        ...
        if not os.path.exists(os.path.join(feature_save_dir, "negative_features_trai2.npy")) or args.overwrite is True:
            negative_clips_train = [str(i) for i in Path(negative_train_output_dir).rglob("*.wav")]*config["augmentation_rounds"]
            random.seed(42)  # (tùy chọn) để tái lập kết quả, nếu không cần thì bỏ dòng này
            random.shuffle(negative_clips_train)
            # negative_clips_train_generator 
            negative_clips_train_data_generator = augment_clips(negative_clips_train, total_length=config["total_length"],
                                                           batch_size=config["augmentation_batch_size"],
                                                        #    background_clip_paths=background_paths,
                                                           RIR_paths=rir_paths,
                                                           noise_paths=config["noise_paths_train"],
                                                           augmentation_probabilities=config["augmentation_probabilities"],
                                                           noise_type_probabilities=config["noise_type_probabilities"],
                                                           save_debug_path=config['save_debug_path_negative_train'],
                                                           snr_ranges=snr_ranges)
            ...
            if n_cpus is None:
                ...
                n_cpus = n_cpus//2
            compute_features_from_generator(negative_clips_train_data_generator, n_total=len([str(i) for i in Path(negative_train_output_dir).rglob("*.wav")])*config["augmentation_rounds"],
                                            clip_duration=config["total_length"],
        ...
        if not os.path.exists(os.path.join(feature_save_dir, "negative_features_test2.npy")) or args.overwrite is True:
            negative_clips_test = [str(i) for i in Path(negative_test_output_dir).rglob("*.wav")]*config["test_augmentation_rounds"]
            # negative_clips_test_generator
            random.seed(42)  # (tùy chọn) để tái lập kết quả, nếu không cần thì bỏ dòng này
            random.shuffle(negative_clips_test)
            negative_clips_test_data_generator = augment_clips(negative_clips_test, total_length=config["total_length"],
                                                          batch_size=config["augmentation_batch_size"],
                                                        #   background_clip_paths=background_paths,
                                                          RIR_paths=rir_paths,
                                                          noise_paths=config["noise_paths_test"],
                                                          augmentation_probabilities=config["augmentation_probabilities"],
                                                          noise_type_probabilities=config["noise_type_probabilities"],
                                                          save_debug_path=config['save_debug_path_negative_test'],
                                                           snr_ranges=snr_ranges)
            logging.info("#"*50 + "\nComputing openwakeword features for generated samples\n" + "#"*50)
            n_cpus = os.cpu_count()
            if n_cpus is None:
                ...
                n_cpus = n_cpus//2
            compute_features_from_generator(negative_clips_test_data_generator, n_total=len([str(i) for i in Path(negative_test_output_dir).rglob("*.wav")])*config["test_augmentation_rounds"],
                                                clip_duration=config["total_length"],
        ...
        #         n_cpus = n_cpus//2
        #     if not os.path.exists(os.path.join(feature_save_dir, "positive_features_trai2.npy")) or args.overwrite is True:
        #         compute_features_from_generator(positive_clips_train_generator, n_total=len([str(i) for i in Path(positive_train_output_dir).rglob("*.wav")])*config["augmentation_rounds"],
        #                                         clip_duration=config["total_length"],
        #                                         output_file=os.path.join(feature_save_dir, "positive_features_train.npy"),
        #                                         device="gpu" if torch.cuda.is_available() else "cpu",
        #                                         ncpu=n_cpus if not torch.cuda.is_available() else 1)
        ...
        #         n_cpus = n_cpus//2
        #     if not os.path.exists(os.path.join(feature_save_dir, "positive_features_test2.npy")) or args.overwrite is True:
        #         compute_features_from_generator(positive_clips_test_generator, n_total=len([str(i) for i in Path(positive_test_output_dir).rglob("*.wav")])*config["test_augmentation_rounds"],
        #                                         clip_duration=config["total_length"],
        #                                         output_file=os.path.join(feature_save_dir, "positive_features_test.npy"),
        #                                         device="gpu" if torch.cuda.is_available() else "cpu",
        #                                         ncpu=n_cpus if not torch.cuda.is_available() else 1)
        ...
        #         n_cpus = n_cpus//2
        #     if not os.path.exists(os.path.join(feature_save_dir, "negative_features_trai2.npy")) or args.overwrite is True:
        #         compute_features_from_generator(negative_clips_train_generator, n_total=len([str(i) for i in Path(negative_train_output_dir).rglob("*.wav")])*config["augmentation_rounds"],
        #                                         clip_duration=config["total_length"],
        #                                         output_file=os.path.join(feature_save_dir, "negative_features_train.npy"),
        #                                         device="gpu" if torch.cuda.is_available() else "cpu",
        #                                         ncpu=n_cpus if not torch.cuda.is_available() else 1)
        ...
        #         n_cpus = n_cpus//2
        #     # if not os.path.exists(os.path.join(feature_save_dir, "positive_features_trai2.npy")) or args.overwrite is True:
        #     #     compute_features_from_generator(positive_clips_train_generator, n_total=len(os.listdir(positive_train_output_dir))*config["augmentation_rounds"],
        #     #                                     clip_duration=config["total_length"],
        #     #                                     output_file=os.path.join(feature_save_dir, "positive_features_train.npy"),
        #     #                                     device="gpu" if torch.cuda.is_available() else "cpu",
        #     #                                     ncpu=n_cpus if not torch.cuda.is_available() else 1)
        #     # if not os.path.exists(os.path.join(feature_save_dir, "negative_features_trai2.npy")) or args.overwrite is True:
        #     #     compute_features_from_generator(negative_clips_train_generator, n_total=len(os.listdir(negative_train_output_dir))*config["augmentation_rounds"],
        #     #                                     clip_duration=config["total_length"],
        #     #                                     output_file=os.path.join(feature_save_dir, "negative_features_train.npy"),
        #     #                                     device="gpu" if torch.cuda.is_available() else "cpu",
        #     #                                     ncpu=n_cpus if not torch.cuda.is_available() else 1)
        #     # if not os.path.exists(os.path.join(feature_save_dir, "positive_features_test2.npy")) or args.overwrite is True:
        #     #     compute_features_from_generator(positive_clips_test_generator, n_total=len(os.listdir(positive_test_output_dir))*config["test_augmentation_rounds"],
        #     #                                     clip_duration=config["total_length"],
        #     #                                     output_file=os.path.join(feature_save_dir, "positive_features_test.npy"),
        #     #                                     device="gpu" if torch.cuda.is_available() else "cpu",
        #     #                                     ncpu=n_cpus if not torch.cuda.is_available() else 1)
        #     if not os.path.exists(os.path.join(feature_save_dir, "negative_features_test2.npy")) or args.overwrite is True:
        #         compute_features_from_generator(negative_clips_test_generator, n_total=len([str(i) for i in Path(negative_test_output_dir).rglob("*.wav")])*config["test_augmentation_rounds"],
        #                                         clip_duration=config["total_length"],
        #                                         output_file=os.path.join(feature_save_dir, "negative_features_test.npy"),
        #                                         device="gpu" if torch.cuda.is_available() else "cpu",
        #                                         ncpu=n_cpus if not torch.cuda.is_available() else 1)
    ...
    if args.train_model is True:
        F = utils.AudioFeatures(device='cpu')
        input_shape = np.load(os.path.join(feature_save_dir, "positive_features_test2.npy")).shape[1:]
        ...
        # 🧩 Nếu có config chỉ định checkpoint — load model + optimizer để train tiếp
        if "resume_checkpoint" in config and config["resume_checkpoint"]:
            ckpt_path = Path(config["resume_checkpoint"])
            if ckpt_path.exists():
                logging.info(f"🔁 Loading checkpoint from {ckpt_path} to continue training...")
                print(f"🔁 Loading checkpoint from {ckpt_path} to continue training...")
                try:
                    checkpoint = torch.load(ckpt_path, map_location="cpu", weights_only=False)
                    continue_train = True
                except Exception:
                    logging.info("⚠️ Safe load failed, retrying with weights_only=False ...")
                    print("⚠️ Safe load failed, retrying with weights_only=False ...")
                    checkpoint = torch.load(ckpt_path, map_location="cpu", weights_only=False)

                # Load model state
                if "model_state_dict" in checkpoint:
                    oww.model.load_state_dict(checkpoint["model_state_dict"])
                    logging.info("✅ Model weights loaded.")
                    print("✅ Model weights loaded.")

                # Load optimizer state (nếu có)
                if "optimizer_state_dict" in checkpoint:
                    oww.optimizer.load_state_dict(checkpoint["optimizer_state_dict"])
                    tmp_op = str(checkpoint["optimizer_state_dict"])
                    logging.info(f"✅ Optimizer state loaded. {tmp_op}")
                    print(f"✅ Optimizer state loaded. {tmp_op}")

                    learning_rate = oww.optimizer.param_groups[0]['lr']

                # Ghi nhận lại các metric (nếu có)
                if "val_recall" in checkpoint:
                    oww.best_val_recall = checkpoint["val_recall"]
                if "val_accuracy" in checkpoint:
                    oww.best_val_accuracy = checkpoint["val_accuracy"]

                logging.info(f"📈 Resume training with best recall={oww.best_val_recall:.4f}, accuracy={oww.best_val_accuracy:.4f}")
                print(f"📈 Resume training with best recall={oww.best_val_recall:.4f}, accuracy={oww.best_val_accuracy:.4f}")

                if "resume_all_checkpoint" in config and config["resume_all_checkpoint"]:
                    for i in Path(config["resume_all_checkpoint"]).glob("*.pt"):
                        tmp_checkpoint = torch.load(i, map_location="cpu", weights_only=False)
                        print(tmp_checkpoint.get("step", 0),checkpoint.get("step", 0))
                        print(tmp_checkpoint.get("phase", 0),checkpoint.get("phase", 0))
                        logging.info(tmp_checkpoint.get("step", 0),checkpoint.get("step", 0))
                        logging.info(tmp_checkpoint.get("phase", 0),checkpoint.get("phase", 0))
                        if tmp_checkpoint.get("step", 0) != checkpoint.get("step", 0) or tmp_checkpoint.get("phase", 0) != checkpoint.get("phase", 0):
                            temp_model = copy.deepcopy(oww.model)
                            temp_model.load_state_dict(tmp_checkpoint["model_state_dict"])
                            oww.best_models.append(temp_model)
                            oww.best_model_scores.append({
                                "training_step_ndx": tmp_checkpoint.get("step", 0),
                                "val_recall": tmp_checkpoint.get("val_recall", 0.0),
                                "val_accuracy": tmp_checkpoint.get("val_accuracy", 0.0),
                                "val_fp_per_hr": tmp_checkpoint.get("val_fp_per_hr", 0.0)
                            })
                    
                logging.info(f"📈 Resume training with {len(oww.best_models)} previous best models")
                print(f"📈 Resume training with {len(oww.best_models)} previous best models")
            else:
                logging.info(f"⚠️ Resume checkpoint not found: {ckpt_path}")
                print(f"⚠️ Resume checkpoint not found: {ckpt_path}")

        oww.model.to(oww.device)
        for state in oww.optimizer.state.values():
            for k, v in state.items():
                if isinstance(v, torch.Tensor):
                    state[k] = v.to(oww.device)
        ...
        config["feature_data_files"]['positive'] = os.path.join(feature_save_dir, "positive_features_trai2.npy")
        config["feature_data_files"]['adversarial_negative'] = os.path.join(feature_save_dir, "negative_features_trai2.npy")
        ...
        X_val_fp = torch.utils.data.DataLoader(
            ...
            batch_size=128#len(X_val_fp_labels)
        )

        X_val_pos = np.load(os.path.join(feature_save_dir, "positive_features_test2.npy"))
        X_val_neg = np.load(os.path.join(feature_save_dir, "negative_features_test2.npy"))
        ...
        X_val = torch.utils.data.DataLoader(
            ...
            batch_size=128#len(labels)
        )
        
        if continue_train:
            # Run re auto training
            logging.info("📈 Resume training with steps={}, learning_rate={}".format(config["steps"],learning_rate))
            print("📈 Resume training with steps={}, learning_rate={}".format(config["steps"],learning_rate))
            best_model = oww.re_auto_train(
                X_train=X_train,
                X_val=X_val,
            ...
            # Run auto training
            logging.info("📈 Training new model with steps={}, learning_rate={}".format(config["steps"],learning_rate))
            print("📈 Training new model with steps={}, learning_rate={}".format(config["steps"],learning_rate))
    ...
    if args.export_checkpoint:
        if not args.checkpoint_path or not args.onnx_output_path:
            raise ValueError("⚠️ Missing required arguments: --checkpoint_path and --onnx_output_path")

        # Khởi tạo model với tham số giống lúc train
        model = Model(
            n_classes=config.get("n_classes", 1),
            input_shape=tuple(config.get("input_shape", (16, 96))),
            model_type=config.get("model_type", "dnn"),
            layer_dim=config.get("layer_size", 128)
        )

        model.export_from_checkpoint(
            checkpoint_path=args.checkpoint_path,
            output_path=args.onnx_output_path,
            class_mapping="output"
        )
        sys.exit(0)

    # Do Data Augmentation
    if args.embedding_clips is True:
        drop_up=True
        if (not os.path.exists(os.path.join(feature_save_dir, "positive_features_trai2.npy")) or args.overwrite is True) and os.path.exists(box_positive_train_output_dir):
            if drop_up:
                positive_clips_train = [str(i) for i in Path(box_positive_train_output_dir).rglob("*.wav") if 'gainup_' not in str(i)]
            else:
                positive_clips_train = [str(i) for i in Path(box_positive_train_output_dir).rglob("*.wav")]
            # print(positive_clips_train[1])
            print(len(positive_clips_train))
            random.seed(42) 
            random.shuffle(positive_clips_train)
            positive_clips_train_generator = embedding_clips(positive_clips_train, total_length=config["total_length"],
                                                           batch_size=config["augmentation_batch_size"])
        else:
            logging.warning("Openwakeword features already exist, skipping data augmentation and feature generation")

        if (not os.path.exists(os.path.join(feature_save_dir, "positive_features_test2.npy")) or args.overwrite is True) and os.path.exists(box_positive_test_output_dir):
            if drop_up:
                positive_clips_test = [str(i) for i in Path(box_positive_test_output_dir).rglob("*.wav") if 'gainup_' not in str(i)]
            else:
                positive_clips_test = [str(i) for i in Path(box_positive_test_output_dir).rglob("*.wav")]
            print(len(positive_clips_test))
            random.seed(42) 
            random.shuffle(positive_clips_test)
            positive_clips_test_generator = embedding_clips(positive_clips_test, total_length=config["total_length"],
                                                          batch_size=config["augmentation_batch_size"])
        else:
            logging.warning("Openwakeword features already exist, skipping data augmentation and feature generation")

        if (not os.path.exists(os.path.join(feature_save_dir, "negative_features_trai2.npy")) or args.overwrite is True) and os.path.exists(box_negative_train_output_dir):
            if drop_up:
                negative_clips_train = [str(i) for i in Path(box_negative_train_output_dir).rglob("*.wav") if 'gainup_' not in str(i)]
            else:
                negative_clips_train = [str(i) for i in Path(box_negative_train_output_dir).rglob("*.wav")]
            random.seed(42) 
            random.shuffle(negative_clips_train)
            negative_clips_train_generator = embedding_clips(negative_clips_train, total_length=config["total_length"],
                                                           batch_size=config["augmentation_batch_size"])
        else:
            logging.warning("Openwakeword features already exist, skipping data augmentation and feature generation")

        if (not os.path.exists(os.path.join(feature_save_dir, "negative_features_test2.npy")) or args.overwrite is True) and os.path.exists(box_negative_test_output_dir):
            if drop_up:
                negative_clips_test = [str(i) for i in Path(box_negative_test_output_dir).rglob("*.wav") if 'gainup_' not in str(i)]
            else:
                negative_clips_test = [str(i) for i in Path(box_negative_test_output_dir).rglob("*.wav")]
            random.seed(42) 
            random.shuffle(negative_clips_test)
            negative_clips_test_generator = embedding_clips(negative_clips_test, total_length=config["total_length"],
                                                          batch_size=config["augmentation_batch_size"])
        else:
            logging.warning("Openwakeword features already exist, skipping data augmentation and feature generation")
        ...
        if (not os.path.exists(os.path.join(feature_save_dir, "positive_features_trai2.npy")) or args.overwrite is True) and os.path.exists(box_positive_train_output_dir):
            ...
            if (not os.path.exists(os.path.join(feature_save_dir, "positive_features_trai2.npy")) or args.overwrite is True) and os.path.exists(box_positive_train_output_dir):
                if drop_up:
                    n_total = len([str(i) for i in Path(box_positive_train_output_dir).rglob("*.wav") if 'gainup_' not in str(i)])
                    compute_features_from_generator(positive_clips_train_generator, 
                                                    n_total=n_total,
                                                    clip_duration=config["total_length"],
                                                    output_file=os.path.join(feature_save_dir, "positive_features_train.npy"),
                                                    device="gpu" if torch.cuda.is_available() else "cpu",
                    ...
                    n_total = len([str(i) for i in Path(box_positive_train_output_dir).rglob("*.wav")])
                    compute_features_from_generator(positive_clips_train_generator, n_total=n_total,
                                                    clip_duration=config["total_length"],
                                                    output_file=os.path.join(feature_save_dir, "positive_features_train.npy"),
                                                    device="gpu" if torch.cuda.is_available() else "cpu",
                                                    ncpu=n_cpus if not torch.cuda.is_available() else 1)
        ...
        if (not os.path.exists(os.path.join(feature_save_dir, "positive_features_test2.npy")) or args.overwrite is True) and os.path.exists(box_positive_test_output_dir):
            ...
            if (not os.path.exists(os.path.join(feature_save_dir, "positive_features_test2.npy")) or args.overwrite is True) and os.path.exists(box_positive_test_output_dir):
                if drop_up:
                    n_total = len([str(i) for i in Path(box_positive_test_output_dir).rglob("*.wav") if 'gainup_' not in str(i)])
                    compute_features_from_generator(positive_clips_test_generator, 
                                                n_total=n_total,
                                                clip_duration=config["total_length"],
                                                output_file=os.path.join(feature_save_dir, "positive_features_test.npy"),
                                                device="gpu" if torch.cuda.is_available() else "cpu",
                    ...
                    n_total = len([str(i) for i in Path(box_positive_test_output_dir).rglob("*.wav")])
                    compute_features_from_generator(positive_clips_test_generator, n_total=n_total,
                                                    clip_duration=config["total_length"],
                                                    output_file=os.path.join(feature_save_dir, "positive_features_test.npy"),
                                                    device="gpu" if torch.cuda.is_available() else "cpu",
                                                    ncpu=n_cpus if not torch.cuda.is_available() else 1)
        ...
        if (not os.path.exists(os.path.join(feature_save_dir, "negative_features_trai2.npy")) or args.overwrite is True) and os.path.exists(box_negative_train_output_dir):
            ...
            if (not os.path.exists(os.path.join(feature_save_dir, "negative_features_trai2.npy")) or args.overwrite is True) and os.path.exists(box_negative_train_output_dir):
                if drop_up:
                    n_total = len([str(i) for i in Path(box_negative_train_output_dir).rglob("*.wav") if 'gainup_' not in str(i)])
                    compute_features_from_generator(negative_clips_train_generator, 
                                                    n_total=n_total,
                                                    clip_duration=config["total_length"],
                                                    output_file=os.path.join(feature_save_dir, "negative_features_train.npy"),
                                                    device="gpu" if torch.cuda.is_available() else "cpu",
                    ...
                    n_total = len([str(i) for i in Path(box_negative_train_output_dir).rglob("*.wav")])
                    compute_features_from_generator(negative_clips_train_generator, n_total=n_total,
                                                    clip_duration=config["total_length"],
                                                    output_file=os.path.join(feature_save_dir, "negative_features_train.npy"),
                                                    device="gpu" if torch.cuda.is_available() else "cpu",
                                                    ncpu=n_cpus if not torch.cuda.is_available() else 1)
        ...
        if (not os.path.exists(os.path.join(feature_save_dir, "negative_features_test2.npy")) or args.overwrite is True) and os.path.exists(box_negative_test_output_dir):
            ...
            if (not os.path.exists(os.path.join(feature_save_dir, "negative_features_test2.npy")) or args.overwrite is True) and os.path.exists(box_negative_test_output_dir):
                if drop_up:
                    n_total = len([str(i) for i in Path(box_negative_test_output_dir).rglob("*.wav") if 'gainup_' not in str(i)])
                    compute_features_from_generator(negative_clips_test_generator, 
                                                    n_total=n_total,
                                                    clip_duration=config["total_length"],
                                                    output_file=os.path.join(feature_save_dir, "negative_features_test.npy"),
                                                    device="gpu" if torch.cuda.is_available() else "cpu",
                    ...
                    n_total = len([str(i) for i in Path(box_negative_test_output_dir).rglob("*.wav")])
                    compute_features_from_generator(negative_clips_test_generator, n_total=n_total,
                                                    clip_duration=config["total_length"],
                                                    output_file=os.path.join(feature_save_dir, "negative_features_test.npy"),
                                                    device="gpu" if torch.cuda.is_available() else "cpu",
                                                    ncpu=n_cpus if not torch.cuda.is_available() else 1)
    ...
    if args.only_augment_clips is True:
        snr_ranges = snr_configs[args.snr_mode]
        # if config['save_raw_positive_test']:
            # shutil.copytree(positive_test_output_dir, config['save_raw_positive_test'], dirs_exist_ok=True)
            # shutil.copytree(negative_test_output_dir, config['save_raw_negative_test'], dirs_exist_ok=True)
        ...
        if not os.path.exists(os.path.join(feature_save_dir, "positive_features_trai2.npy")) or args.overwrite is True:
            positive_clips_train = [str(i) for i in Path(positive_train_output_dir).rglob("*.wav")]*config["augmentation_rounds"]
            # positive_clips_train
            only_augment_clips(positive_clips_train, total_length=config["total_length"],
                                                           batch_size=config["augmentation_batch_size"],
                                                        #    background_clip_paths=background_paths,
                                                           RIR_paths=rir_paths,
                                                           noise_paths=config["noise_paths_train"],
        ...
        if not os.path.exists(os.path.join(feature_save_dir, "positive_features_test2.npy")) or args.overwrite is True:
            positive_clips_test = [str(i) for i in Path(positive_test_output_dir).rglob("*.wav")]*config["test_augmentation_rounds"]
            # positive_clips_test_generator
            only_augment_clips(positive_clips_test, total_length=config["total_length"],
                                                          batch_size=config["augmentation_batch_size"],
                                                        #   background_clip_paths=background_paths,
                                                          RIR_paths=rir_paths,
                                                          noise_paths=config["noise_paths_test"],
        ...
        if not os.path.exists(os.path.join(feature_save_dir, "negative_features_trai2.npy")) or args.overwrite is True:
            negative_clips_train = [str(i) for i in Path(negative_train_output_dir).rglob("*.wav")]*config["augmentation_rounds"]
            # negative_clips_train_generator 
            only_augment_clips(negative_clips_train, total_length=config["total_length"],
                                                           batch_size=config["augmentation_batch_size"],
                                                        #    background_clip_paths=background_paths,
                                                           RIR_paths=rir_paths,
                                                           noise_paths=config["noise_paths_train"],
        ...
        if not os.path.exists(os.path.join(feature_save_dir, "negative_features_test2.npy")) or args.overwrite is True:
            negative_clips_test = [str(i) for i in Path(negative_test_output_dir).rglob("*.wav")]*config["test_augmentation_rounds"]
            # negative_clips_test_generator
            only_augment_clips(negative_clips_test, total_length=config["total_length"],
                                                          batch_size=config["augmentation_batch_size"],
                                                        #   background_clip_paths=background_paths,
                                                          RIR_paths=rir_paths,
                                                          noise_paths=config["noise_paths_test"],
