...
class Model():
    ...
    def predict_clip(self, clip: Union[str, np.ndarray], padding: int = 1, chunk_size=1280, **kwargs):
        ...
        if isinstance(clip, str):
            ...
            # Attempt 1: Try standard wave module (fastest)
            try:
            ...
            # Attempt 2: Try soundfile (no external DLL dependencies)
            if data is None:
                try:
                    import soundfile as sf
                    audio_data, sr = sf.read(clip, dtype='float32')
                    if len(audio_data.shape) > 1:
                        audio_data = audio_data.mean(axis=1)  # stereo -> mono
                    data = (np.clip(audio_data.flatten(), -1.0, 1.0) * 32767).astype(np.int16)
                except Exception:
                    pass
            
            # Attempt 3: Try scipy (alternative fallback)
            if data is None:
                try:
                    from scipy.io import wavfile
                    sr, audio_data = wavfile.read(clip)
                    if len(audio_data.shape) > 1:
                        audio_data = audio_data.mean(axis=1)  # stereo -> mono
                    data = audio_data.astype(np.int16)
                except Exception:
                    pass
            
            # Attempt 4: Try torchaudio (may fail due to torchcodec DLLs)
            if data is None:
                try:
                    waveform, sr = torchaudio.load(clip)
                    if waveform.size(0) > 1:
                        waveform = waveform.mean(dim=0)  # stereo -> mono
                    data = (np.clip(waveform.numpy().flatten(), -1.0, 1.0) * 32767).astype(np.int16)
                except Exception:
                    pass
            
            # Final fallback: Create silent audio if all methods fail
            if data is None:
