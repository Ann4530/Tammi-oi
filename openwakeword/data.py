...
class SpeedPerturbation:
    def __init__(self, min_speed_rate=0.9, max_speed_rate=1.1, p=0.7):
        """
        Thay đổi tốc độ phát âm thanh ngẫu nhiên (tăng/giảm).
        Sử dụng torchaudio.transforms.Resample để giữ nguyên cao độ.
        """
    ...
    def __call__(self, y, sr):
        ...
        ----------
        y : torch.Tensor
            Tensor âm thanh dạng [num_samples] hoặc [1, num_samples]
        sr : int
            Sample rate (ví dụ: 16000)
        """
        # Không augment nếu random > p
        if random.random() >= self.p:
            return y

        # Chọn tốc độ ngẫu nhiên
        rate = random.uniform(self.min_speed_rate, self.max_speed_rate)

        # Đảm bảo đầu vào có đúng shape [1, num_samples]
        if y.ndim == 1:
            y = y.unsqueeze(0)

        new_sr = int(sr * rate)

        # ⚙️ Resample để mô phỏng tăng tốc / giảm tốc
        resampled = torchaudio.functional.resample(y, orig_freq=sr, new_freq=new_sr)

        # Đưa về lại đúng sample rate ban đầu
        resampled = torchaudio.functional.resample(resampled, orig_freq=new_sr, new_freq=sr)

        # Đảm bảo vẫn là tensor float32 trong [-1, 1]
...
def normalize_amplitude(audio_segment, target_dBFS=-15.0):
    """
    Chuẩn hóa (hoặc tăng/giảm ngẫu nhiên) âm lượng của AudioSegment
    về mức dBFS trong khoảng [min_dBFS, max_dBFS].

    Parameters
    ----------
    audio_segment : AudioSegment
        Đoạn âm thanh cần chuẩn hóa
    min_dBFS : float
        Mức dBFS tối thiểu
    max_dBFS : float
        Mức dBFS tối đa
    ...
    -------
    AudioSegment
        Âm thanh sau khi được điều chỉnh âm lượng
    """
    change_in_dBFS = target_dBFS - audio_segment.dBFS
    return audio_segment.apply_gain(change_in_dBFS)

def filter_audio_files(paths):
    AUDIO_EXTS = ('.wav', '.flac', '.mp3', '.ogg', '.m4a')
    return [p for p in paths if p.lower().endswith(AUDIO_EXTS)]
...
def augment_clips(
        ...
        noise_type_probabilities: dict = {},
        RIR_paths: List[str] = [],
        noise_paths: dict = {},
        save_debug_path = None,
        snr_ranges = {"music": (-20,15), "speech": (-20,20), "noise": (-10,20)}
            # snr_ranges = {"music": (-10,15), "speech": (-10,20), "noise": (-10,20)}
            # snr_ranges = {"music": (-8,15), "speech": (-8.5,20), "noise": (-10,20)}
        ):
    """
    ...
    RIR_paths = filter_audio_files(RIR_paths)
    saved_count = 0
    speed_augment = SpeedPerturbation(min_speed_rate=1,max_speed_rate=1.5,p=augmentation_probabilities.get("Speed",0.0))
    speed_augment2 = SpeedPerturbation(min_speed_rate=1,max_speed_rate=1.3,p=augmentation_probabilities.get("Speed",0.0))
    ...
    for i in range(0, len(clip_paths), batch_size):
        ...
        for clip in batch:
            try:
                ...
                if clip_sr != sr:
                    ...
                    clip_data = resampler(clip_data)
                    clip_sr = sr

                clip_data_np = speed_augment(clip_data.squeeze(0),clip_sr)
                clip_data_np = clip_data[0].numpy()
                

                clip_data_np = create_fixed_size_clip(torch.from_numpy(clip_data_np), total_length, clip_sr)

                audio_segment = pydub.AudioSegment(
                    (clip_data_np * 32767).astype(np.int16).tobytes(),
                    frame_rate=sr,
                    sample_width=2,   # 2 byte = 16-bit
                    channels=1        # 1 = mono (sửa nếu stereo)
                )

                if random.random() < 0.7:
                    target_dBFS = random.uniform(-50,-30)
                else:
                    target_dBFS = random.uniform(-30,-15)

                normalized_seg = normalize_amplitude(audio_segment, target_dBFS=target_dBFS)
                normalized_samples = np.array(normalized_seg.get_array_of_samples()).astype(np.float32)
                normalized_np = normalized_samples / 32768.0

                augmented_clips.append(torch.from_numpy(clip_data_np))
            except:
                try:
                    clip_data, clip_sr = torchaudio.load(clip)
                    if clip_sr != sr:
                    ...
                    clip_data_np = speed_augment2(clip_data.squeeze(0),clip_sr)
                    clip_data_np = clip_data[0].numpy()
        ...
        augmented_batch = torch.vstack(augmented_clips).to(device)

        if np.random.random() < augmentation_probabilities.get("AddBackgroundNoise",0.0):
            # snr_ranges = {"music": (-20,15), "speech": (-20,20), "noise": (-10,20)}
            # snr_ranges = {"music": (-10,15), "speech": (-10,20), "noise": (-10,20)}
            # snr_ranges = {"music": (-8,15), "speech": (-8.5,20), "noise": (-10,20)}
            noise_options = {}
            if (noise_paths or {}).get("music"): noise_options["music"] = (noise_paths or {}).get("music")
            if (noise_paths or {}).get("speech"): noise_options["speech"] = (noise_paths or {}).get("speech")
            if (noise_paths or {}).get("noise"): noise_options["noise"] = (noise_paths or {}).get("noise")

            if noise_options:
                available_noise_types = list(noise_options.keys())
                if noise_type_probabilities:
                    population = [nt for nt in noise_type_probabilities if nt in available_noise_types]
                    weights = [noise_type_probabilities[p] for p in population]
                    if abs(sum(weights) - 1) > 1e-6:
                        total_weight = sum(weights)
                        weights = [w/total_weight for w in weights]
                    chosen_noise_type = np.random.choice(population, p=weights)
                else:
                    chosen_noise_type = random.choice(available_noise_types)
            
            chosen_paths = noise_options[chosen_noise_type]
            min_snr, max_snr = snr_ranges[chosen_noise_type]

            noise_adder = torch_audiomentations.AddBackgroundNoise(background_paths=chosen_paths,min_snr_in_db=min_snr,
                                                                max_snr_in_db=max_snr,p=1,mode="per_batch",sample_rate=sr)
            
            augmented_batch = noise_adder(samples=augmented_batch.unsqueeze(1), sample_rate=sr).squeeze(1)

        if np.random.random() < augmentation_probabilities.get("RIR",0.0) and RIR_paths:
            rir_waveform, sr_rir = torchaudio.load(random.choice(RIR_paths))
            if sr!=sr_rir:
                rir_waveform = torchaudio.transforms.Resample(sr_rir, sr)(rir_waveform)
            augmented_batch = reverberate(augmented_batch.cpu(),rir_waveform,rescale_amp="avg")

        final_batch_np = (augmented_batch.cpu().numpy()*32767).astype(np.int16)
        valid_indices = np.where(final_batch_np.max(axis=1)!=0)[0]
        final_batch_to_yield = final_batch_np[valid_indices]

        # original_filenames = [os.path.basename(p) for p in batch]
        # for e, sample_to_save in enumerate(final_batch_to_yield):
        #     original_name, _ = os.path.splitext(original_filenames[e])
        # save_debug_count=10
        
        if save_debug_path: # and saved_count < save_debug_count:
            original_filenames = [os.path.basename(p) for p in batch]
            num_to_save_this_batch = len(final_batch_to_yield)
            for j in range(num_to_save_this_batch):
                try:
                    sample_to_save = final_batch_to_yield[j]
                    original_name, _ = os.path.splitext(original_filenames[j])
                    debug_filename = f'batch_{i//batch_size}_sample_{j}_{original_name}_augmented.wav'
                    full_debug_path = os.path.join(save_debug_path, debug_filename)
                    
                    sf.write(full_debug_path, sample_to_save, sr)
                except:
                    try:
                        sample_to_save = final_batch_to_yield[j]
                        original_name, _ = os.path.splitext(original_filenames[j])
                        debug_filename = f'batch_{i//batch_size}_sample_{j}_{original_name[:38]}_augmented.wav'
                        logging.warning(f"File name is too long: {original_name}")
                        full_debug_path = os.path.join(save_debug_path, debug_filename)
...
def only_augment_clips(
    ...
    RIR_paths = filter_audio_files(RIR_paths)
    saved_count = 0
    speed_augment = SpeedPerturbation(min_speed_rate=0.9,max_speed_rate=1.6,p=augmentation_probabilities.get("Speed",0.0))

    # Iterate through all clips and augment them
    for i in tqdm(range(0, len(clip_paths), batch_size)):
        batch = clip_paths[i:i+batch_size]
        augmented_clips = []
        for clip in batch:
            clip_data, clip_sr = torchaudio.load(clip)
